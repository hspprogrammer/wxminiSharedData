...
\[2023.5.\] `A`: 新增[debug 字段](#other)，用于开启/关闭 setState 时的 console。
...
store 中 onShareAppMessage 返回值的优先级是`次于`页面级的，所以当 Page 中有 onShareAppMessage 且有返回值，则会优先使用 Page 中的分享。
...
```html
<view bindtap="goAnyWhere" data-url="/index/index"> 首页 </view>
```
...
用于清空全局状态，使所有$state 下任意的状态为 undefined。
